...
V2.1 不改变 24 周主线、阶段顺序和最终能力目标，只对执行风险进行工程性加固：

1. 第 1 周正式采用 UE 5.8 源码构建，并锁定引擎分支或 Commit、Visual Studio、MSVC Toolset 与 Windows SDK。
2. 日志类别随业务模块出现再创建；第 1 周只创建 `LogOutpost` 与 `LogOutpostEditor`。
3. `Builds/`、`SavedProfiling/`、`TestReports/` 默认不进入普通 Git 历史；最终交付物使用 Release、CI Artifact 或明确的 LFS 规则保存。
4. 第 1 周新增 `VerifyWeek01.bat`，验证清理、重编译、打包、退出码与 Editor 模块隔离。
5. 第 4 周加入 Listen Server + Client 的最小网络冒烟测试，为第 13 周 GAS 预测和第 14 周系统网络化建立前置认知。
...
├─ SavedProfiling/          # 本地 Trace；默认不进入普通 Git 历史
├─ Builds/                  # 本地打包产物；默认不进入普通 Git 历史
├─ TestReports/             # 本地测试报告；由 CI Artifact 或发布包保存
├─ README.md
└─ DemoVideo.mp4
```

版本控制原则：源码、配置、脚本、文档和项目内容资产进入仓库；可重建的大体积产物默认忽略。需要长期保留的最终包、Trace 或报告，应通过 Release、CI Artifact、对象存储，或经过评估后的 Git LFS 规则保存。
...
## 第 1 周：工程、源码引擎、构建、版本控制与第一份可运行包

### 核心目标

建立可以持续迭代六个月的项目底座，而不是立即堆 Gameplay 功能；同时在项目开始阶段完成 UE 5.8 源码引擎构建，避免到 Dedicated Server 阶段再进行高风险迁移。

### 学习内容

- UE 源码仓库、Release Branch / Tag、Commit 锁定。
- `Setup.bat`、`GenerateProjectFiles.bat`、`UE5.sln` 与源码引擎编译链路。
...
1. 完成 UE 5.8 源码引擎构建：
   ...
   → 启动 UnrealEditor.exe。
2. 记录并锁定：
   Engine Branch / Tag / Commit
   Visual Studio 版本
   MSVC Toolset
   Windows SDK。
3. 使用该源码引擎创建 Networked Outpost C++ 项目。
4. 创建 OutpostRuntime 和 OutpostEditor。
5. 配置 Public / Private 目录。
6. 第 1 周只创建：
   LogOutpost
   LogOutpostEditor。
   Ability、Network、AI、UI 日志在对应模块真正出现时再创建。
7. 建立 Git LFS、.gitignore 与提交规范。
8. 明确 Builds/、SavedProfiling/、TestReports/ 的保存策略。
9. 添加一张启动地图和最小 GameMode。
10. 完成第一次 Development 打包。
11. 编写 BuildDevelopment.bat。
12. 编写 VerifyWeek01.bat。
```
...
Builds/Week01/                 # 本地产物，不默认提交普通 Git
```

### `VerifyWeek01.bat` 最低职责

```text
检查 UE_ROOT 与项目路径
→ 清理项目 Binaries / Intermediate
→ 重新编译 Development Editor Target
→ 执行 Development BuildCookRun
→ 检查进程退出码
→ 检查目标可执行文件存在
→ 检查打包目录未包含 OutpostEditor / UnrealEd 相关二进制
→ 输出成功或失败摘要
```

### 验收

- 源码引擎能够从 `Engine/Binaries/Win64/UnrealEditor.exe` 启动。
- 能说明 `Setup.bat`、`GenerateProjectFiles.bat` 和编译 UE5 Target 分别完成什么工作。
- 删除项目的 Intermediate 和 Binaries 后可以重新生成项目并编译。
- OutpostEditor 不进入 Runtime 包。
- 打包程序可以启动到目标地图。
- `VerifyWeek01.bat` 在成功时返回 0，在失败时返回非 0。
- 能解释 Build.cs 与 Target.cs 的职责差异。
- 能解释为什么 `.uasset` 冲突不能像文本代码一样直接合并。
- `Builds/`、`SavedProfiling/`、`TestReports/` 没有无意进入普通 Git 历史。
...
- 研究源码引擎与 Installed Build 的差别，但本周不要求制作 Installed Build。
...
- 输入语义与 Gameplay 命令分离。
- 客户端请求与权威执行的设计。
- Listen Server、Client、Authority、Ownership、Server RPC、RepNotify 的最小认知。
- 自动化测试基础。
...
核心 Gameplay 仍可先以单机纵切实现，但 API 必须按照服务器权威设计：
...
同时增加一个独立的最小网络冒烟测试，不要求本周把完整交互系统网络化：

```text
Listen Server + 1 Client
→ Client 在具有正确 Ownership 的对象上调用 Server RPC
→ Server 修改一个 Replicated 属性
→ Client 通过 RepNotify 观察结果
→ 临时移除 Ownership，记录 RPC 为什么无法按预期到达
```

不能把验证和执行全部写在 UI 或输入回调里，也不能把“客户端调用了 Server RPC”误认为客户端拥有修改权威状态的权限。
...
Tests/NetworkSmokeTestChecklist.md
```
...
- Listen Server + Client 冒烟测试能够证明 Server RPC、Ownership、属性复制和 RepNotify 的最小链路。
- 能解释该冒烟测试只是网络前置认知，不等于第 14～17 周的完整网络化已经完成。
...
```text
第 1 周记录 UE Release Branch / Tag / Commit。
升级或同步引擎代码后重新运行 GenerateProjectFiles.bat。
引擎升级必须使用独立分支，并完成编译、Cook、核心测试和打包验证后再合并。
不在主项目中直接追踪 ue5-main 的每日变化。
```

## 构建、Trace 与报告产物管理

```text
Builds/、SavedProfiling/、TestReports/ 默认加入 .gitignore。
最终交付包通过 Release、CI Artifact 或对象存储保存。
确需版本化的少量 Trace 或二进制报告，单独制定 Git LFS 规则。
文档必须记录产物对应的 Commit、构建配置、测试环境和复现命令。
```
...
- 日志和调试工具。
- 基础测试。
- Listen Server + Client 的 Server RPC / Ownership / RepNotify 冒烟测试。
...
- [Downloading Unreal Engine Source Code](https://dev.epicgames.com/documentation/unreal-engine/downloading-source-code-in-unreal-engine)
- [Building Unreal Engine from Source](https://dev.epicgames.com/documentation/unreal-engine/building-unreal-engine-from-source)
- [Generate Project Files for Your IDE](https://dev.epicgames.com/documentation/unreal-engine/how-to-generate-unreal-engine-project-files-for-your-ide)
- [Create an Installed Build](https://dev.epicgames.com/documentation/unreal-engine/create-an-installed-build-of-unreal-engine)
